# Extracted from: akshayyelne/Home-Care-AI-Product-Management/.claude/skills/Execution/Artifact_21_PRD.md
# Generated: 2026-07-31T00:49:45.152Z

**Project:** Home-Care-AI
**Stage:** Execution → Stage A (The Definition — Intent)
**Skill:** create-prd
**Date:** 2026-03-27
**Methodology:** 8-section PRD template; SMART OKRs; Priority 1/2/3 feature classification
...
|---|---|---|
| **HS-STRAT-01** | Artifact 15 §6 (L3 interventions) + §4 (Moment of Truth) | Moment of Truth → P1 feature; all L3 interventions → P1 features (§7.2) |
| **HS-STRAT-02** | Artifact 16 (Compliance NFRs) | Mitigation Requirements → Constraints §7.4 and NFR table |
| **HS-STRAT-03** | Artifact 20 §12 (Token budget + model selection) | Haiku for L1/L2, Sonnet for L3 → §7.3 Technology |
| **HS-STRAT-04** | Artifact 14 §6 "What After" OKRs | OKR-1 through OKR-6 → §4 Objectives (exact, no substitution) |
| **HS-STRAT-05** | Artifact 19 §1 Geoffrey Moore statement | → §3 Background problem definition; §5 market segment |

**Regulatory Context:** Australian Privacy Act 1988 (APP). HIPAA-grade security as design floor. All data processing in AWS ap-southeast-2 (Sydney). No cross-border transmission in v1.
...
| **OKR-6** ⚠️ | Knowledge survivability | New coordinator effective in **< 5 days** with SPP handover | 2–3 months shadowing | Agency owner assessment at 30 days | Within 30 days of coordinator onboarding |

> ⚠️ **OKR-6 note:** Baseline is directional — not sourced from a named agency owner interview. Will be updated after XP-2A agency owner interviews. OKR-6 does not gate Sprint 1.
...
| Trust / preference matching | High (coordinator knows) — but unsystem atised | None | **High — SPP-driven, transferable** |
...
```
1. [PUSH NOTIFICATION]
   "Jenny called in sick — 3 visits affected.
   Replacements ready for review."
   [Tap to open → ]

2. [VACANCY OVERVIEW SCREEN]
   Three visit cards: Mrs. Kim (8:00 AM) · Arthur Kovacs (9:30 AM) · Lin Chen (11:00 AM)
   Each card shows: Client name · Visit time · SPP summary (familiarity threshold label)
   [Tap Mrs. Kim's card → ]

3. [CANDIDATE SHORTLIST — Mrs. Kim]
   TOP MATCH: David Kim
   ✓ Qualified  ·  2.1 km  ·  2 prior visits with Mrs. Kim
   [Secondary: Sarah Ng — ✓ Qualified · 4.3 km · 0 prior visits]
   [Third: Marco Torres — ✓ Qualified · 5.8 km · 1 prior visit]
   [Tap David Kim → ]

4. [★ MOMENT OF TRUTH — COORDINATOR APPROVAL CARD]
   David Kim
   ┌───────────────────────────────────┐
   │ ✓ Qualified for Mrs. Kim's care   │
   │ 📍 2.1 km — Surry Hills           │
   │ ★ 2 prior visits with Mrs. Kim   │  ← FAMILIARITY FLAG (largest element, DR-1)
   └───────────────────────────────────┘
   If you approve, this is what will be sent:
   → David: "CareBridge needs you to cover a visit in Surry Hills at 8 AM. Reply YES/NO."
   → Mrs. Kim: "Your visit today will be with David Kim, who has visited you before."
   → Margaret (daughter): "Your mother's visit today will be covered by David Kim..."
      [Sent AFTER Mrs. Kim is notified — guaranteed]
   [APPROVE] ← single tap

5. [RESOLUTION SCREEN]
   ✓ David Kim assigned — awaiting reply
   ✓ Mrs. Kim notified — SMS delivered
   ✓ Margaret Chen notified — SMS delivered
...
- **P1 — Must ship v1:** Product does not exist without these. A Level 3 intervention absent from this list breaks the safety chain (CLAUDE.md HS-STRAT-01). An OKR without its P1 feature cannot be measured.
- **P1.1 — Post-beachhead (60 days after first paying customer):** Specifically requested by beachhead users; deferred to protect Sprint 1 focus.
- **P2 — v2 (after beachhead validation + compliance unlock):** Requires a v2 unlock condition per Artifact 17 §07.


#### Feature 1: Soft Preference Profile (SPP) Data Model + Capture Flow [P1]
...
**OKR it measures:** OKR-4 (≥ 80% of active clients with ≥ 3 SPP fields populated within 90 days).


#### Feature 2: Smart Match Engine [P1]
...
#### Feature 3: 3-Tap Coordinator Approval Flow — The Moment of Truth [P1]
...
**What ACT-A-01 must display (non-negotiable UI elements — from HS-STRAT-05h):**
1. Familiarity count: "2 prior visits with Mrs. Kim" — this exact format, this exact prominence
2. Qualifications confirmed badge: "✓ Qualified for [client first name]'s care requirements"
3. Full notification preview: what message David will receive, what Mrs. Kim will receive, what the family will receive — in that order, with the E-3 note: "[Family name] will be notified after [client name]"
...
#### Feature 4: Automated Carer Assignment + Briefing [P1 — L3 Actions]
...
**ACT-C-01 — Carer Assignment SMS:**
Template: `"Hi [carer_first_name], [agency_name] needs you to cover a visit for a client in [client_suburb] at [visit_time]. Reply YES to confirm or NO if unavailable."`
...
**ACT-C-02 — Carer Briefing SMS (sent after carer confirms):**
Template assembled from SPP structured fields:
...
IF SPP empty for this client:
    SEND minimal briefing ("Client prefers a calm, steady approach")
...
#### Feature 5: Client + Family Notification Pipeline with E-3 Gate [P1 — L3 Actions]
...
**ACT-P-01 — Client Notification SMS:**
Template (phrasing branch on P-3 familiarity threshold):
...
**ACT-P-01 note on P-3 ≥ 2 (cognitively impaired clients):**
G-DS-05 guard: clients with P-3 familiarity threshold = "known carers only" must never receive the phrase "who has visited you before" if the assigned carer is not actually known. The phrasing branch above enforces this, but the `agentic-logic-spec` must assert this guard explicitly.
...
IF ACT-P-01 delivery fails:
    SET client_notified = false
    ALERT coordinator: "[client_first_name] has no notification channel — please call them directly before family is notified."
```

**ACT-F-01 — Family Notification SMS:**
Template: `"Hi [family_contact_first_name], [agency_name] is letting you know that [client_first_name]'s visit today will be covered by [carer_first_name], who has visited [client_pronoun] before. Arranged by [coordinator_first_name]. — [agency_name]"`
...
BEFORE ACT-F-01:
    ...
    IF client_notified = false:
        SUPPRESS ACT-F-01
        ALERT coordinator: "Family notification held — [client_first_name] has not yet been notified. Notify them manually to release."
...
#### Feature 6: Immutable Audit Log [P1 — Non-Negotiable]
...
#### Feature 7: Vacancy Unresolved Escalation [P1 — L3 Edge Case]
...
#### Feature 8: Compliance Gap Dashboard [P1.1 — Post-Beachhead, 60 Days After Launch]
...
#### Feature 9: WhatsApp Notifications [P2 — v2, Blocked Pending CRIT-01 Unlock]
...
#### Feature 10: AlayaCare Bi-Directional Integration [P2 — Blocked Pending AX-02]
...
**Architecture rule (from Artifact 17 CRIT-02 — binding):**
...
|---|---|---|---|
| **CRIT-01** | All v1 notifications via AU-hosted SMS gateway only. No WhatsApp/Twilio/Meta-hosted channel in v1. | Artifact 17 §02 | Channel Wrapper architecture. `channel_config.v1 = 'SMS_AU'`. Hardcoding any channel is prohibited. |
| **CRIT-02** | All external content (notifications, briefings) generated by template interpolation only. No LLM call generates text sent to carers, clients, or families. | Artifact 17 §03 | `content_generation_method == TEMPLATE_INTERPOLATION` asserted in agentic-logic-spec. Any LLM assignment to notification actions = CRITICAL defect. |
| **CRIT-03** | P-2 (gender preference) carries zero weight in the matching algorithm. Displayed as advisory only: "Client preference (advisory — not scored)." | Artifact 17 §04 | `E1_LEGAL_SIGNOFF = false` constant in code. `ASSERT P2 NOT IN scoring_weights`. Guard logged as G-CC-4. |
...
1. **AX-01:** Engineer confirms in writing that `free_text_notes` field does not exist in the v1 SPP schema and no unstructured text field of any name exists in the v1 data model. (CRIT-04 completion.)
2. **AX-02 result known:** Either bi-directional AlayaCare integration is scoped (v2 roadmap item) or scoping is deferred. Either answer is fine — the unknown is the problem.
3. **Privacy Counsel briefed:** On DPIA-01 scope and timeline. Sprint 1 can begin before DPIAs are complete, but the counsel relationship must be active.
4. **Design wireframes for 3-Tap Approval Flow (sketch-gate):** The Moment of Truth screen cannot be built from spec alone. Minimum acceptable fidelity: PM Lead-approved sketch showing element order, DR-1 font-size annotations, tap target dimensions ≥ 44×44px, and notification preview above Approve button. Full Figma polish required before E1 but not before Wave 4 coding start (Artifact 24 US-10 AC-6).
5. **Copy Approval (B1-B):** Privacy Counsel confirms all four SMS template strings (ACT-C-01, ACT-C-02, ACT-P-01, ACT-F-01 — Artifact 23 §4.1) are APP-compliant. Angela (CC-001) has reviewed all four for coordinator and client clarity. Templates are committed to `config/sms_templates.py` with `approved_by` and `approved_date` fields set. No notification story passes `harness-audit-grader` SD-01B until this gate is closed.
...
| **HIGH-03** | DPIA for CC-1 compound (P-3 + P-4 + P-5) — when initiated? | Privacy Counsel | Sprint 1 initiation | Cannot collect compound CC-1 data without DPIA |
